...
|----|----------|---------|------------|
| INF-01 | LOW | **SSL certificate verification disabled on DB connection** — Supabase's Session Pooler presents a self-signed certificate in its chain that Python's `ssl.create_default_context()` (system CA store) rejects. `ssl.CERT_NONE` was initially used as a workaround. | **Fixed:** `database.py` now uses `certifi`'s Mozilla CA bundle (`cafile=certifi.where()`). `CERT_REQUIRED` is restored; only `check_hostname` is disabled (pooler SNI may not match cert CN). The connection is both encrypted and certificate-verified. |
